...
| **2** 🟧 | **Tilt the floor/walls (live).** DONE: `scenes/base.draw_terrain_tilted` warps the visible floor window (wrap-aware) + extrudes wall boxes; `draw_world` branches at pitch>0 with skybox + decorations; actors stand via Phase-1 projection + a sin(pitch) lift; camera pivots about screen centre; **F3** debug-toggles pitch 0↔55, eased. Gated pixel-identical at pitch 0. Proven against wrap seams + folds. **FINE-TUNE (open):** actors/decorations don't depth-interleave with or get occluded by walls; folds use flat placement under tilt; enemies/projectiles still on the legacy offset; window scale 1.0 (no zoom-out); actor-lift is a flat constant. | med | the tilt lands (F3) |
