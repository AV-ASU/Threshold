...
Vane): **a POSSESSION gate hides the row** (you cannot ask about a thing you
are not holding), but a **TRUST or disposition gate does NOT** — the question
is askable and the character refuses it in their own voice, naming what would
...
### 2. **[Fable + Opus]** The favor economy — the requests variant
...
**Fences:** rewards stay **incommensurable** (no dominant pick); **never
evidence**; the ripple is **mood, not a meter**; it **never gates an
ending**; **existing verbs only** (give via E / dialog).
...
+ per-material styles, Phases 2 and 3) are complete (`CHANGELOG.md`, "Walls &
interior geometry"). What is left is the north star.

- **Phase 4 — freeform walls:** a wall SEGMENT primitive off the tile grid,
  unlocking diagonal walls, a curved apse, a round silo/tower.
...
- **Cross-cutting:** thinner walls occlude less, so re-derive interior cover
  as the segment lands; extend `tests/stealth.py` §16; VISION toward the
  Darkwood organic read.

### 5. **[Opus]** Stealth — the deferred verbs and the tight bands
...
- **Widen `store_row`'s WEST band.** `tools/band_gaps.py` measures what the
  ...
  alone — it is the §13 lost-space MOUTH, and tight is the point there. The
...
### 6. **[Opus]** Combat — the stun window and the difficulty toggle
...
nudge (he ran the route, goods in and out) — his newspaper exchange already
carries some of this; confirm it's enough or add the nudge. **[Opus]** put
his picked-clean semi in **`royce_yard`**, his own lot off `lane_end`, which
is both his household and the far end of the town's street chain (optional
...
- **Cut the revisit-nudges** (`_REVISIT_NUDGES`, the "I should go back and
  ask him" appends). They are the clearest instance of the grievance, and #1
  is built on the opposite principle: a question that is simply THERE when he
  arrives is the same beat without the hand-holding.
...
Put the standing stones on **`river_road`**, the length of bank the road
actually runs beside and the scene that already carries the Preacher's
remains, with a worn cult path to them. Pre-cult dressing: lichened,
...
Mask grammar) + a single `notes` beat (never evidence, no cosmology). Gives
SEAL its precedent without a word.
...
doctrine that keeps it coherent: **light is safety from the small things and
...
anything (the `SUS_CONCEAL_DARK` shadow-cover pass was CUT — see #25). The
player's tool is the beam they point, not the square they stand on. Every
item here is judged against that: a light verb that makes a tile safe is the
wrong verb.
...
- **The cult response to a dark room** (rides #23): the camp PROCESSES to a
  room going dark and fans out, a staged response rather than an alarm.
- **The capture fork is RULED: capture is GAME OVER.** The CAPTURED card
  ...
  that cultists must EARN that ending — it lands through #5/#6, not a new
  system.
...
- **23b. The town half.** The **yield**: a local a cult patrol passes steps
  ...
  sprint flinches or hurries indoors (rides homebody `_inside`); a kill
  nearby empties the street for the visit. Strictly mundane reactions only.
- **23c. The mechanical pieces.** SEARCH **sweep partition** (multiple
  searchers divide `sweep_points`, no duplicate checks); **room posture** (a
  per-scene calm/uneasy/roused int raised by shots, struggles, found bodies,
  decaying; modulates walk speed, scan time, sweep budget — ships with its
  constants set conservatively, tuned alongside the rest of `SUS_*`); the
  **flank call** (a locked chaser pulls at most one nearby patrol to a flank
  point, same LOS/suspicion rules, normal search timer, never soft
  omniscience); **object-state investigation** (a left-on noisemaker, an
  opened door: pause at it, mark the room uneasy).
- **23d. Content passes (Fable).** Fuller local **day-loops** on the JOBS
...
- **15 INSIDE + 15 OUTSIDE.** The fifteen above are the INTERIOR library; a
  matching EXTERIOR fifteen follows (porch ensemble, lean-to + stacked wood,
  clothesline, rain barrel + gutter, fence-and-gate runs, mailbox post, burn
  barrel, machine shed face, pump + trough, chicken wire pen, propane/fuel
  tank, junk pile, tarped equipment, porch chair + rifle, antler-over-door).
  Same rules. The exterior half now has eleven yards to serve, so it carries
  more weight than it did when the town was one map.
...
- **ANIMATE WHERE IT EARNS IT.** Each ensemble asks: could a part move, and
  ...
  behavior rides the genset link (#21). A still room with ONE moving thing is
  scarier than a busy one.
- **EVERY ENSEMBLE SHIPS ITS WEAR LAYER, WORLD-SPACE.** The object's years
  ...
  projected geometry in its true plane; screen-aligned marks are error class 7.
...
- **The Mask is a PART** (the 18th), player-scale and a real 3D object that
  turns a full 360 — never a billboard. **One Mask storm-wide, MIGRATING**,
  and the hop is EARNED, not on a dwell timer.
...
1. **THE AMALGAM'S CATCH ANIMATION.** The apex's death card is a wordless
   ...
   built from the amalgam grammar rather than the Unfolding's — the parts,
   the gold cuts, the Mask coming in. Wordless by design, so there is no
   label and nothing owed to `DIALOGUE.md`. Land it through a VISION look
   pass.
2. **Retire THE UNFOLDING.** Rewire the roaming King's own card and the
...
3. **The last canon reconciliation.** NARRATIVE §8 describes the walking King
   as the thing that reaches you because that is what ships until (2);
   "one pursuit, two shapes" collapses back to one shape when the Unfolding
   retires.
...
### 26. **[Opus + Fable]** The lost spaces — the dark manipulation layer
...
1. **Asymmetric return.** The way back is not the way you came. Today the
   ...
   not a detour.
2. **Breathe with threat.** The space stretches as the meter fills: the exit
   light's 6-20 tile band, the reshuffle rate, and the field's feature
   density all read the visibility meter, so being hunted makes the walk
   longer.
3. **More mouths.** One mouth on one scene is a vertical slice. Open the
   remaining dark, non-wrapping edges that earn it — the street verges
   already derive WHICH field they lead to (`_VERGE_LOST`), so this is
   `set_lost_edge` calls plus a look pass per edge.
4. **Per-chunk generation + a silent re-origin** for a truly endless walk
   (today: a large finite bound + spawn-at-centre), with landmark/exit
   generation moving per-chunk with it.
5. **The ev-warp variants:** the field swaps for a longer / warped /
   more-hostile version with evidence, plus richer linear features (fences,
   ruined buildings you cannot enter).
6. **Interiors for the yard buildings that have none.** Mrs. Calder, Royce
   and Garrick got small one-room interiors with the yard layer; the other
   households have their exteriors only. Give the ones a scene actually sends
   you into a room, through #24's library.
...
### 28. **[Opus + Fable]** Cut the calendars
...
- **Guards:** `tests/flow.py` asserts several Pell/paper beats; update them in
  the same commit.
...
becomes work when a scene and a composition are named — #4 is this pass aimed
at outdoor dread and has both, so it is the live instance.

### 8. Parked — terrain megabuilds
...
hill in #4 is the cheap substitute that covers most of what the warp was for.
...
- **[Fable + Opus]** **Rev. Asa Crane murder reveal + sprite** — the
  ...
  **bespoke sprite** — the current corpse is a placeholder medieval knight
  ...
  collar, cross in the mess). (2) Stage the approach (wrongness before sight,
  long sightline). Art only; lore unchanged.
...
- **[Opus]** **Mine retrofit tail cleanups** (none player-visible) — (a)
  ...
  `_RACK_CHARS` into the shared wall-scan char-set plumbing instead of a third
  parallel set; (c) the cave `door_style` key list in `scenes/__init__.py`
  duplicates the `UNDERGROUND_SCENES` gating idea, derive from one source;
  (d) `husk_bundle` + `pillar` are registered kinds with no placements (keep
  as reusable art or cut).
