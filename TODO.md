...
4. **THE ROAD SHOULD GENERATE, AND THE HOUSES ARE FOUND ON IT** (maintainer,
   2026-07: *"once we're on a safe path, those paths generate infinitely and
   the town's houses are just another thing you find on the path"*). The town
   is authored today as a fixed row of streets you walk in order, which is a
   map you can learn. It should not be. The road runs on as you walk it, a
   stretch that has been generated stays generated, and a yard is something
   you come across. **There is exactly one of each yard.** Replaces the
   earlier idea of rearranging the fixed streets into a nicer shape: that was
   still a map.
