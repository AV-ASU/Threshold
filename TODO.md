...
- **[Opus]** **Redecoration-audit deferred polish** *(2026-07 17-scene visual audit; every MEDIUM+ finding was fixed, these LOW items were deliberately deferred)* — (a) bell tower: ~~a bespoke timber bell-stock frame (A-frame uprights + yoke) instead of the scaled table platform~~ **DONE (2026-07), then CORRECTED (2026-07):** new `bell_stock` SOLID_PROP (`rendering/props.py _draw_bell_stock_solid` -- a braced timber trestle: four canted posts to a headstock beam, a mid-height ledger RING and N-S top caps so it carries horizontal members in BOTH planes, diagonal knee braces on every face, iron gudgeon straps) replaces the `scale=2.0` table box; same 2x2 collision footprint, the bell hangs off the headstock. **The first build ran every beam E-W (planar in X) and collapsed to a thin diagonal stick edge-on from E/W** -- a defect that slipped through because the ad-hoc four-facing render script was silently drawing the NORTH facing four times (the `camera.yaw` omission, root-caused + fixed in VISION.md's render recipe). Rebuilt with ledger/top members in both planes and GENUINELY re-verified as a bell frame from all four facings; gate green, docstring reconciled. **Still open in (a):** louvered belfry openings instead of the glazed cottage windows (the `'i'` window char renders glazed everywhere, so a belfry louver needs its own window style or a wall-deco louver over the openings -- a small mechanism, deferred); ~~(b) schoolhouse: jitter the 12 grid-locked cots a few px so the dormitory reads crammed, not installed~~ **DONE (2026-07):** each cot is nudged a few px off its tile (DRAW only, seeded per tile; the collision footprint stays tile-locked), so the two banks read shoved-in by hand, not an installed grid; VISION-verified + gate green; ~~(c) hanging_figure legibility: the sprite reads as a standing hooded blob among trees, not a suspended body~~ **DONE (2026-07):** `_draw_hanging_figure` redrawn -- a long taut rope up out of frame, a lolled head, a limp TAPERING torso (was a robe flaring wider at the hem, the standing read), limp arms held in, and feet DANGLING toes-down clear of the ground, the whole form swinging as a PENDULUM from the fixed anchor (the feet swing furthest). VISION-verified in the clearing (reads as suspended now); gate green; ~~(d) lodge: cluster the missing-flyer/polaroid "wall of the vanished" above the reception desk instead of interleaving with trophies~~ **DONE (2026-07):** the three flyers grouped tight (cols 9-11, above the desk) so they read as one wall; the buck / fish / portrait moved to their own stretches (the portrait off to col 13). **Canon check:** the flyers were KEPT, not cut -- `missing_flyer` is intentional surface town-dread dressing (also in brimley + the lodge yard), the community searching for people the fold won't give back; that is a DIFFERENT read from the underground Sorting-Hall "wall of the vanished" that was cut as a killer-cult tracking read, so the surface version stays. VISION-verified; gate green; ~~(e) lodge_hall: bump the side-table footprint and re-home the corner-crammed sampler~~ **DONE (2026-07):** the sampler moved off the crammed NE corner (col 21, jammed against the loft stairs) to a clear north-wall bay between the doors (col 12), pairing with the portrait as its comment always claimed; the hall side table (nightstand) footprint bumped w26 h30 -> w36 h26 so it reads as a proper console, not a cramped nightstand. VISION-verified; gate green; ~~(f) guest_room_a: nudge the buck trophy toward a light source~~ **MOOT (2026-07):** `guest_room_a` is a `SAFE_SCENE` -- flat-lit, so `_draw_dark`'s light-pool model never runs and the buck reads uniformly lit wherever it hangs (there is no pool to nudge it into); its centred north-wall placement is fine. No change (documented, revisit only if the room ever leaves SAFE_SCENES); ~~(g) farmhouse: the "phantom marks on the walls" comment vs floor-decal render (stale comment or wall kind swap)~~ **DONE (2026-07):** resolved as a STALE COMMENT, not a wall swap -- `phantom_mark` is a `_FLOOR_DECAL_KINDS` decal used game-wide (it warps onto the floorboards under the tilt and reads fine there, VISION-checked in the farmhouse), so the farmhouse comment AND the misleading `_draw_phantom_mark` draw-comment ("scratched into a wall") were corrected to say floor. A game-wide wall-deco swap was deliberately NOT done (big blast radius across ~10 scenes; revisit only if the maintainer wants marks up on the walls). **The redecoration-audit deferred-polish phase is now COMPLETE (a-g all resolved 2026-07):** (a-e) fixed, (f) moot, (g) resolved as a comment fix. The ONE deferred remainder is (a)'s louvered belfry openings (needs a per-scene window-style mechanism, tracked in (a) above). Each fix rode the full gate; note that the original four-facing captures were compromised by the `camera.yaw` render-recipe bug (north four times) and were re-run genuinely once it was found -- only (a) hid a real E/W defect, since (b)/(c)/(d)/(g) are facing-invariant standees / wall-decos / floor-decals and (f) is a no-op, so a north-only capture had in fact been adequate for those.
