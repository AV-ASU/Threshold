...
| **The Preacher — Rev. Asa Crane** | **Local — innocent dissenter** | A small-town minister who **names the cult from his own pulpit** — oblivious to *what* they truly are, but loud that they're no church. **His doom is a chosen beat now (2026-07, the ask-verb fork):** his flock exchange works him hot, and the PI either **presses him** — `preacher_doomed` latches, and the PI's culpability is filed as a NOTE (`crane_provoked`) — or **holds him back**, which only *banks the fire*: the question stays open to press later, and when the case stalls at two surface threads the PI's interior voice points him back at the pulpit (`the_third_thread` nudge — the investigation needs what the naming costs). He cannot be saved, only un-pointed (§1b: the cult cannot be talked out of anything; Crane dies for believing he can). **The doom sends him OUT (2026-07 rework):** he leaves his pulpit and goes down to the river after his flock, believing they can be talked home. The church stands empty on the next entry (the one-shot river-mud line points at the water), and his body is found **on the Brimley riverbank**, gutted, his cross in the viscera (evidence #4) — never on his own church floor. His knowledge stays a LOCAL's: he never speaks of the underground; all he ever had was the rumor the boy Toby told him — that they walked off down the river one night. The town murders the ones who name them. |
...
| 4 | **The Preacher** | his **cross** (item `cross`) taken from the viscera | The **Brimley riverbank** (`brimley`) — he went to the water after his flock; the emptied church (`church`) points there | The town murders the ones who name them — gutted for preaching against the cult |
...
| **Break the rite before sealing** — *YOU FUCKED UP* (a game over) | You tear down the rite *in place* — the obvious heroic move — with the source still open. The lid comes off a pressurized pot: His influence floods out, uncontained, here and now | Ending sequence, **wordless by design (locked 2026-07)**: you destroy the altar/Sign/kneeling, one breath of quiet, then the flood. No card, no line — the cage-for-Him truth is learnable beforehand through the cult's own notes, never stated by the ending | `_play_ending("rite_broken")`. Triggered at the **Sign Chamber altar** (the FIRST place you meet the active rite): a choice — *take the mask* (controlled) vs *tear it down* (the trap). |
...
> the shaft you stopped being sure whether you mean to end it, or only to
...
- **Item keys are load-bearing** (saves and game logic depend on them):
  `mom_notebook`, `pallid_mask` (until 2026-07 `sigil_rubbing`; the disk
  loader migrates), `cult_calling`, `cult_bargain`,
  `cult_digging`, `cross`, `robe`,
  `unsent_letter`, `flashlight`, `lumber_axe`, `woodshed_key`,
  `rite_envelope`, `chalk`, `incense`. *(The `rope` key is CUT with the
  rope mechanic — the descent is the rite now.)*
